        ...
        if(validator.validateUUID(uuid)){
            ...
            if(profile_response.isRight()){
                ...
                if(profile){
                    ...
                    logError(profile_response.value, 'FriendsController', 'PlayerProfile.getFriends')
                    return res.sendStatus(500)
                }
                return res.json(getJsonError(10, {values: { uuid }}))
            }
            logError(profile_response.value, 'FriendsController', 'PlayerProfileRepository.search')
            return res.sendStatus(500)
        }
        ...
        if(validator.validateUUID(uuid) && validator.validateUUID(friend_uuid)){
            ...
            if(profile_response.isRight()){
                ...
                if(profile){
                        ...
                        if(friend_profile){
                            if(await profile.areFriends(friend_profile.uuid)){
                                ...
                                if(remove_in_profile_response.isLeft()){
                                    logError(remove_in_profile_response.value, 'FriendsController', 'FriendsListRepository.removeFriend')
                                    return res.sendStatus(500)
                                }
                                ...
                                if(remove_in_friend_response.isLeft()){
                                    logError(remove_in_friend_response.value, 'FriendsController', 'FriendsListRepository.removeFriend')
                                    return res.sendStatus(500)
                                }
                    ...
                    logError(friend_profile_response.value, 'FriendsController', 'PlayerProfileRepository.search')
                    return res.sendStatus(500)
                }
                return res.json(getJsonError(10, {values: { uuid }}))
            }
